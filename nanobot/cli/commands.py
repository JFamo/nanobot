...
def gateway(
    ...
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
    api_key: str | None = typer.Option(None, "--api-key", envvar="NANOBOT_API_KEY", help="API key for gateway auth"),
):
    ...
    bot_id_env = os.environ.get("BOT_ID", "")
    ...
    async def run():
        ...
        async def handle_agent_run_stream(request):
            ...
            async def _on_message(msg) -> None:
                await queue.put({"type": "message", "content": msg.content})
        ...
        async def handle_sync_scopes(request):
            body = await request.json()
            try:
                agent.sync_scoped_tools(
                    body.get("google_scopes", []),
                    body.get("x_scopes", []),
                    body.get("github_scopes", []),
                )
        ...
        async def handle_consolidate_session(request):
            ...
            if not session_key:
                return web.json_response(
                    {"success": False, "error": "session_key is required"}, status=400,
                )
        ...
        async def handle_identity(request):
            ...
            try:
                if soul_path.exists():
                    ...
                    m = _re.search(r"^# Soul\s*\n\s*I am (\S+)", text, _re.MULTILINE)
                    if not m:
                        m = _re.match(r"\s*I am (\S+)", text)
                    if m:
        ...
        web_app = web.Application(middlewares=middlewares)
        ...
        runner_http = web.AppRunner(web_app)
        await runner_http.setup()
        await web.TCPSite(runner_http, "0.0.0.0", port).start()
